...
const CAT_SIZES = {
  Football:   ['3','4','5'],
  Basketball: ['6','7'],
  Running:    ['6','7','8','9','10','11'],
  Boxing:     ['S','M','L','XL'],
  Swimming:   ['S','M','L','XL'],
  Tennis:     ['One Size'],
  Fitness:    ['Standard','One Size'],
  Cycling:    ['S','M','L'],
};
...
function openAddProductModal() {
  ...
  document.getElementById('prod-cat').value = 'Football';
  // Reset image
...
function openEditProductModal(id) {
  ...
  document.getElementById('prod-cat').value       = p.cat       || 'Football';
  // Image
...
async function saveProduct() {
  ...
  await db_upsertProduct({ id, emoji, name, brand, cat, price: priceVal, oldPrice, badge, desc, sizes: CAT_SIZES[cat] || ['One Size'], imageUrl });
...
function openStockModal(productId) {
  ...
  const sizes = CAT_SIZES[p.cat] || ['One Size'];
...
function setAllSizes() {
  ...
  (CAT_SIZES[p.cat] || ['One Size']).forEach(size => {
